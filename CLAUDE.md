...
- `server/utils/roster.ts`: owns `RaiderIoMember`, `RosterMember` and `toRosterMembers()`, the pure filter-map-sort the route applies to Raider.IO's response. Separate from the route so it can be tested without a Nitro runtime, since a route module runs `defineCachedEventHandler` the moment it is imported- `server/utils/warcraftlogs.ts`: the Warcraft Logs v2 client. Holds the OAuth token in module scope, runs one GraphQL query per call, and throws a 503 when the credentials are missing so a page can say "not configured" instead of "down". Also owns `GUILD` and the difficulty-integer map
