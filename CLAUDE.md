...
- `server/utils/raidNights.ts`, `server/utils/raidDetail.ts`: `fetchRaidNights(tierId)` (every guild log in one tier, plus the loggers' personal logs that are guild nights: at most 30 players and at least 8 of them on the current roster, which keeps pugs out; the current tier is cached an hour, a finished tier a week, since one costs about 130 API points) and `fetchRaid(code)`, the Warcraft Logs fetches behind the raid pages, as cached functions refreshed once an hour. In `server/utils` rather than in the routes so the character pages can reuse them. `server/utils/raids.ts` holds the pure `groupRaidNights()`: logs that overlap (two raiders logging at once) or follow within three hours (a night uploaded in parts) are one night, counted across all its logs and linking to the most complete one
