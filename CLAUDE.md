...
- `app/pages/characters/[realm]/[name].vue`, `server/api/characters/[realm]/[name].get.ts`: a page for every character on the current guild roster, and only them: anyone else, a pug from a raid log say, gets the same 404 as an unknown name (`rosterKey()` in `server/utils/roster.ts` matches name and realm however an API spells them). On raid night pages, players off the roster are plain text marked "(pug)". Combines Raider.IO (gear, Mythic+, raid progression, portrait) with Warcraft Logs rankings for one tier (`?tier=<zone id>`, all three difficulties in one aliased query, `hps` for healers) and the guild raid nights the character was in. Cached an hour per character and tier. An opted-out or unknown name is the same 404
