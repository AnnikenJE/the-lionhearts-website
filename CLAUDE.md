...
- `app/pages/privacy.vue`: what the site shows about a character, where it comes from, and how to be taken off it. Required by the Blizzard Developer API Terms of Use, which want a privacy policy, a named data source and an immediate way out
...
- `app/utils/wow.ts`: `classColor()`, the official WoW class colours. Returns a CSS colour, so call sites bind it with `:style`, not `:class`. Normalises the key because Raider.IO says "Death Knight" and Warcraft Logs says "DeathKnight"
...
- `server/utils/optOut.ts`: the list of character names that asked not to appear, plus `isOptedOut()` and `withoutOptedOut()`. Applied in `toRosterMembers()` and on the raid detail route, so one name added to the list removes a character from every page that names one. Removing someone is a one-line edit plus a deploy, and that is deliberate: the terms require the removal to be immediate
- `server/utils/warcraftlogs.ts`: the Warcraft Logs v2 client. Holds the OAuth token in module scope, runs one GraphQL query per call, and throws a 503 when the credentials are missing so a page can say "not configured" instead of "down". Also owns `GUILD` and the difficulty-integer map
- `server/api/raids.get.ts`, `server/api/raids/[code].get.ts`: the two Warcraft Logs routes, cached 15 minutes and 1 hour respectively (a finished log never changes)
