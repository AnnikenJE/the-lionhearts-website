...
- `server/utils/warcraftlogs.ts`: the Warcraft Logs v2 client. Holds the OAuth token in module scope, runs one GraphQL query per call, and throws a 503 when the credentials are missing so a page can say "not configured" instead of "down". Also owns `GUILD`, the difficulty-integer map and `RAID_TIERS`, the guild's raid tiers as Warcraft Logs zone ids, newest first (the first is the default everywhere; adding a tier is one line)
